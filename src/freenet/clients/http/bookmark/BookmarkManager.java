...
public class BookmarkManager implements RequestClient {
		...
		try {
			...
			try {
				if (backupBookmarksFile.exists() && backupBookmarksFile.canRead() &&
					...
					SimpleFieldSet sfs = SimpleFieldSet.readFrom(backupBookmarksFile, false,
						true);
					readBookmarks(MAIN_CATEGORY, sfs);
				} else {
	...
	private class USKUpdatedCallback implements USKCallback {
		...
		public void onFoundEdition(long edition, USK key, ObjectContainer container, ClientContext context,
		                           boolean wasMetadata, short codec, byte[] data, boolean newKnownGood,
		                           boolean newSlotToo) {
			if (! newKnownGood) {
				FreenetURI uri = key.copy(edition).getURI();
				node.makeClient(PRIORITY_PROGRESS, false, false)
					.prefetch(uri, 60 * 60 * 1000, FProxyToadlet.MAX_LENGTH_WITH_PROGRESS, null,
						PRIORITY_PROGRESS);
				return;
			}
			...
			for (int i = 0; i < items.size(); i++) {
				...
				try {
					...
					if (usk.equals(key, false)) {
						if (logMINOR) {
							Logger.minor(this,
								"Updating bookmark for " + furi + " to edition " +
									edition);
						}
						matched = true;
	...
	public void removeBookmark(String path) {
		...
		} else if (((BookmarkItem) bookmark).getKeyType().equals("USK")) {
			try {
			...
			try {
				...
				for (int i = 0; i < nbBookmarks; i++) {
					...
					try {
						...
						String name = (isRoot ? "" : prefix + category.name) + '/' + item
							.name;
						putPaths(name, item);
