...
public final class FProxyToadlet extends Toadlet implements RequestClient {
	...
	public void handleMethodPOST(URI uri, HTTPRequest req, ToadletContext ctx) throws ToadletContextClosedException, IOException, RedirectException {
		...
		if (ks.equals(Path.MAIN.url)||ks.startsWith("/servlet/")) {
			try {
		...
		if (logMINOR) {
			Logger.minor(FProxyToadlet.class,
				"handleDownload(data.size=" + data.size() + ", mimeType=" + mimeType +
					", requestedMimeType=" + requestedMimeType + ", forceDownload=" +
					forceDownload + ", basePath=" + basePath + ", key=" + key);
		}
			...
			if (horribleEvilHack(data) && ! (mimeType.startsWith("application/rss+xml"))) {
				...
				NodeL10n.getBase()
					.addL10nSubstitution(option, "FProxyToadlet.backToFProxy",
						new String[]{"link"},
						new HTMLNode[]{new Link(Path.MAIN.url)});
	...
	private static void addDownloadOptions(ToadletContext ctx, OutputList optionList, FreenetURI key, String mimeType,
		...
		if (threatLevel != PHYSICAL_THREAT_LEVEL.MAXIMUM && !core.isDownloadDisabled()) {
			...
			HTMLNode optionForm = ctx.addFormChild(option, Path.DOWNLOAD.url, "tooBigQueueForm");
			...
			if (threatLevel == PHYSICAL_THREAT_LEVEL.HIGH) {
				...
				//optionForm.addText(l10n("downloadToDiskSecurityWarning") + " ");
			}
		}
	...
	private void innerHandleMethodGET(URI uri, HTTPRequest httprequest, ToadletContext ctx, int recursion)
				...
				if (fr.hasData()) {
					...
					optionList.addItem()
						.addChild(ctx.getPageMaker().createBackLink(ctx,
							l10n("goBackToPrev")));
					optionList.addItem()
						.addLink(Path.MAIN.url, NodeL10n.getBase().getString("Toadlet.homepage"),
		...
		try {
			...
			if (e.newURI != null) {
				...
				optionList.addItem().addLink(Path.MAIN.url, NodeL10n.getBase().getString("Toadlet.homepage"),
					l10n("abortToHomepage"));
				optionList.addItem()
	...
	private static String writeSizeAndMIME(OutputList fileInformationList, FetchException e) {
	...
	private static String writeSizeAndMIME(OutputList fileInformationList, long size, String mime, boolean finalized) {
		if(size > 0) {
			if (finalized) {
				fileInformationList.addItem((l10n("sizeLabel") + ' ') + SizeUtil.formatSize(size));
			} else {
				fileInformationList.addItem((l10n("sizeLabel") + ' ') + SizeUtil.formatSize(size) + l10n("mayChange"));
			}
		} else {
			fileInformationList.addItem(l10n("sizeUnknown"));
		}
		if(mime != null) {
			fileInformationList.addItem(NodeL10n.getBase().getString("FProxyToadlet." + (finalized ? "mimeType" : "expectedMimeType"), new String[]{"mime"}, new String[]{mime}));
		} else {
			fileInformationList.addItem(l10n("unknownMIMEType"));
	...
	public static void maybeCreateFProxyEtc(NodeClientCore core, Node node, Config config,
		...
		server.register(uploadToadlet, Path.UPLOAD, uploadToadlet);
		...
		server.register(seclevels, Path.SECLEVELS, null);

		PproxyToadlet pproxy = new PproxyToadlet(client, node, core);
		server.register(pproxy, Path.PLUGINS, null);
